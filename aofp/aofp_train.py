...
import numpy as np
from aofp.aofp_prune import aofp_prune
...
def aofp_train_main(
        local_rank,
        target_layers, succ_strategy, warmup_iterations, aofp_batches_per_half, flops_func,
...
remain_flops_ratio=0):
    ...
    with Engine(local_rank=local_rank) as engine:
        ...
        layer_to_start_iter = {i: (_init_interval * i + warmup_iterations) for i in target_layers}
        print('the initial layer_to_start_iter = {}'.format(layer_to_start_iter))
        #   0.  get all the AOFPLayers
        layer_idx_to_module = {}
        for submodule in model.modules():
            if hasattr(submodule, 'score_mask') or hasattr(submodule, 't_value'):
        ...
        for epoch in range(done_epochs, cfg.max_epochs):
            ...
            for submodule in model.modules():
                if hasattr(submodule, 'base_mask'):
                    cur_deps[submodule.conv_idx] = np.sum(submodule.base_mask.cpu().numpy() == 1)
            ...
            if local_rank == 0:
                ...
                print('remain flops ratio = ', remain_ratio, 'the target is ', remain_flops_ratio)
                print('##########################')
        ...
        final_deps = aofp_prune(model, origin_deps=cfg.deps, succ_strategy=succ_strategy,
                   save_path=os.path.join(cfg.output_dir, 'finish_pruned.hdf5'))
        origin_flops = flops_func(cfg.deps)
        cur_flops = flops_func(final_deps)
        engine.log('##################################################################')
        engine.log(cfg.network_type)
        engine.log('origin width: {} , flops {} '.format(cfg.deps, origin_flops))
