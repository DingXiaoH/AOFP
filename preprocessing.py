...
from tensorflow.contrib.data.python.ops.prefetching_ops import MultiDeviceIterator
...
CIFAR10_MEAN = [125.30690002, 122.95014954, 113.86599731]   #BGR

def cifar_parse_example_proto(example_serialized):
    feature_map = {
    ...
    return features['image/encoded'], label, None, None
...
class InputPreprocessor(object):
    ...
    def build_multi_device_iterator(self, batch_size, num_splits, cpu_device,
                                    params, gpu_devices, dataset, subset):
        ...
        with tf.name_scope('batch_processing'):
            ...
            ds = self.create_dataset(
                ...
                datasets_sloppy_parallel_interleave=(
                    params.datasets_sloppy_parallel_interleave))
            multi_device_iterator = MultiDeviceIterator(
                ds,
                gpu_devices,
...
class BaseImagePreprocessor(InputPreprocessor):
    ...
    def create_dataset(self,
        ...
        # print('the used tfrecords files are : ', file_names)
        if not file_names:
...
class ImagenetOfficialResNetPreprocessor(RecordInputImagePreprocessor):
...
class ThinetPreprocessor(RecordInputImagePreprocessor):

    def preprocess(self, image_buffer, bbox, batch_position):
        print('using the ThiNet preprocessing')
        if self.train:
            decoded = tf.image.decode_jpeg(
                image_buffer, channels=3, dct_method='INTEGER_FAST')
            distorted_image = tf.image.resize_images(decoded, [256, 256])
            distorted_image = tf.random_crop(distorted_image, [self.height, self.width, 3])
...
class OldAlexPreprocessor(RecordInputImagePreprocessor):
    def preprocess(self, image_buffer, bbox, batch_position):
        print('using the OldAlex preprocessing')
        if self.train:
            decoded = tf.image.decode_jpeg(
                image_buffer, channels=3, dct_method='INTEGER_FAST')
            distorted_image = tf.image.resize_images(decoded, [256, 256])
...
class MNISTTFRecordPreprocessor(RecordInputImagePreprocessor):

    def parse_and_preprocess(self, value, batch_position):
        assert self.supports_datasets()
        image_buffer, label_index, _, _ = cifar_parse_example_proto(value)
        image = self.preprocess(image_buffer, None, batch_position)
        return (image, label_index)

    def preprocess(self, image_buffer, bbox, batch_position):
        image = tf.image.decode_jpeg(image_buffer, channels=1, dct_method='INTEGER_FAST')
        image = tf.cast(image, self.dtype)
        if self.summary_verbosity >= 3:
            print('summary the raw image')
            tf.summary.image('raw.image', tf.expand_dims(image, 0))
        # Note: image is now [height,width,1] with range [0, 255]
        # print('----*****-----*****-----normalize the MNIST images')
        # normalized = normalized_image(image)
        return normalized_image(image)



class CIFARTFRecordBasePreprocessor(RecordInputImagePreprocessor):
    ...
    def preprocess(self, image_buffer, bbox, batch_position):
        ...
        if self.train:
            image = self.pad_method(image)
            print('padded image shape:', image.get_shape())
        ...
        # Note: image is now [height,width,3] with range [0, 255]
        print('----*****-----*****-----normalize the CIFAR images')
        normalized = normalized_image(image)
        return normalized

class CIFARTFRecordStdPreprocessor(CIFARTFRecordBasePreprocessor):

    def pad_method(self, image):
        return tf.image.resize_image_with_crop_or_pad(image, self.height + 8, self.width + 8)

class CIFARTFRecordReflectPreprocessor(CIFARTFRecordBasePreprocessor):

    def pad_method(self, image):
        print('------------- use reflect padding --------------')
        return tf.pad(image, paddings=[[4,4], [4,4], [0,0]], mode='REFLECT')


#TODO ugly design
class CIFARTFRecordOldPreprocessor(RecordInputImagePreprocessor):
...
class SlimInceptionV2Preprocessor(RecordInputImagePreprocessor):
    def preprocess(self, image_buffer, bbox, batch_position):
        # pylint: disable=g-import-not-at-top
        try:
            from research.slim.preprocessing.inception_preprocessing import preprocess_image
        except ImportError:
            tf.logging.fatal('Please include tensorflow/models to the PYTHONPATH.')
            raise
        print('using the slim/inception preprocessing in models-master')
        decoded = tf.image.decode_jpeg(
                image_buffer, channels=3, dct_method='INTEGER_FAST')
        image = preprocess_image(
            decoded, height=self.height, width=self.width, bbox=bbox,
            is_training=self.train)
        summary_ops = tf.get_collection(tf.GraphKeys.SUMMARIES)
        print('the summary op after preprocessing:', summary_ops)

        return tf.cast(image, self.dtype)

class SlimVggPreprocessor(RecordInputImagePreprocessor):
    def preprocess(self, image_buffer, bbox, batch_position):
        # pylint: disable=g-import-not-at-top
        try:
            from research.slim.preprocessing.vgg_preprocessing import preprocess_image
        except ImportError:
            tf.logging.fatal('Please include tensorflow/models to the PYTHONPATH.')
            raise
        print('using the slim/vgg preprocessing in models-master')
        decoded = tf.image.decode_jpeg(
                image_buffer, channels=3, dct_method='INTEGER_FAST')
        image = preprocess_image(
            decoded, output_height=self.height, output_width=self.width,
            is_training=self.train)
        summary_ops = tf.get_collection(tf.GraphKeys.SUMMARIES)
        print('the summary op after preprocessing:', summary_ops)
        return tf.cast(image, self.dtype)
...
class Cifar10ImagePreprocessor(BaseImagePreprocessor):
    ...
    def minibatch(self,
        ...
        print('called minibatch, batchsize=',self.batch_size)
        with tf.name_scope('batch_processing'):
            ...
            for i in xrange(self.batch_size):
                ...
                raw_image = tf.transpose(raw_image, [1, 2, 0])  # HWC format
...
class CIFARStdImagePreprocessor(Cifar10ImagePreprocessor):

    #   no normalize,but subtract the channel-wise mean
    def preprocess(self, raw_image):
        """Preprocessing raw image."""
        print('preprocess CIFAR-10 or 100 images in the std way, do not normalize but subtract the BGR mean')
        if self.summary_verbosity >= 3:
            print('summary the raw image')
            tf.summary.image('raw.image', tf.expand_dims(raw_image, 0))
        if self.train and self.distortions:
